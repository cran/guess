...
test_that("calculate_expected_values matches likelihood formulas for nodk model", {
  ...
  lgk <- 0.3
  lkk <- 0.3
  params <- c(lgg, lgk, lkk)
  total_obs <- 1

  expected <- calculate_expected_values(gamma_i, params, total_obs, "nodk")

  vec <- numeric(4)
  vec[1] <- (1 - gamma_i) * (1 - gamma_i) * lgg
  vec[2] <- (1 - gamma_i) * gamma_i * lgg + (1 - gamma_i) * lgk
  vec[3] <- (1 - gamma_i) * gamma_i * lgg
  vec[4] <- gamma_i * gamma_i * lgg + gamma_i * lgk + lkk
